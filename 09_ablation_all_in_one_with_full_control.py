# -*- coding: utf-8 -*-
r"""
EvoStruct-GAT: Full-model control + 9 ablation experiments, all-in-one runner
==============================================================

Purpose
-------
Run one full-model control plus all nine ablation experiments sequentially from the FINAL
...
# Run Full Model Control + all nine ablations by default.
# Existing completed ablations will be skipped automatically when
# SKIP_COMPLETED = True.
#
# If later you only want to run the full-model control:
# RUN_EXPERIMENT_NUMBERS = [0]
#
# If later you only want selected ablations:
# RUN_EXPERIMENT_NUMBERS = [2, 5, 9]
RUN_EXPERIMENT_NUMBERS = list(range(0, 10))
...
EXPERIMENTS = [
    ...
    # 0. Full-model control
    #
    # This reproduces the formal model inside the same all-in-one
    # implementation used by the nine ablations. No component is removed.
    # Its purpose is to verify that the integrated runner reproduces the
    # formal partner-only DSSP 5 Å result before accepting the ablations.
    AblationConfig(
        number=0,
        key="full_model_control",
        folder="00_full_model_control",
        display_name="Full Model Control",
    ),
...
# in the Full Model Control and all nine ablations, including experiment 6.
...
#
# The flags reproduce the formal architecture and the architecture ablations supplied by
...
def run_one_experiment(
    ...
    # --------------------------------------------------------
    # Critical:
    # reset seed BEFORE each group so the ten standalone-style
    ...
    print(
        (
            "FULL MODEL CONTROL"
            if config.number == 0
            else f"ABLATION {config.number}/9"
        )
    )
...
def main():
    ...
    print(
        "EvoStruct-GAT | Full Model Control + 9 Ablations | All-in-One"
    )
    ...
    # --------------------------------------------------------
    # Run Full Model Control first, then selected ablations in the exact requested order
    # --------------------------------------------------------
    ...
    print(
        "FULL MODEL CONTROL + ALL REQUESTED ABLATION EXPERIMENTS FINISHED"
    )
    ...
    # --------------------------------------------------------
    # Full-model reproducibility diagnostic
    #
    # Known formal partner-only DSSP 5 Å result:
    # AUROC     0.8081
    # F1        0.4562
    # Precision 0.4005
    # Recall    0.5298
    # Accuracy  0.8220
    #
    # This block only prints the difference; it does not alter
    # any result or decide whether an experiment is accepted.
    # --------------------------------------------------------
    if (
        "Order" in summary_df.columns
        and
        (summary_df["Order"] == 0).any()
    ):
        ...
        print(
            "\nFull-model control vs formal 5 Å reference:"
        )

        for metric_name, reference_value in formal_reference.items():
            ...
            print(
                f"{metric_name:9s}: "
                f"control={current_value:.4f} | "
                f"formal={reference_value:.4f} | "
