...
EvoStruct-GAT is a multimodal graph attention model designed for the accurate residue-level prediction of protein-protein interaction (PPI) interfaces[cite: 1]. The model represents each target-chain residue as a graph node, integrates residue-level representations from the ESM-2 protein language model with DSSP-derived structural features, and learns spatial neighborhood information through distance-aware graph attention[cite: 1].
