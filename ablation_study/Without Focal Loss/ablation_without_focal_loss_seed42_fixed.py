...
# ====================== 2. Focal Loss 定义（本消融中不调用，仅保留以便与正式代码对照） ======================
...
# 单因素消融：仅将 Focal Loss 替换为普通 BCE；图级加权采样和动态负节点采样均保留
criterion = nn.BCEWithLogitsLoss()

print("=" * 60)
print(f"Experiment: w/o Focal Loss")
print(f"Random seed: {SEED}")
print(f"Batch size: {BATCH_SIZE}")
print(f"Learning rate: {LEARNING_RATE}")
print(f"Epochs: {EPOCHS}")
print(f"Patience: {PATIENCE}")
print(f"Dynamic negative sampling ratio: 1:{UNDERSAMPLE_RATIO}")
print("Loss: BCEWithLogitsLoss")
print("Graph-level WeightedRandomSampler: enabled")
print("=" * 60)
...
# ====================== 5. 训练循环 ======================
print("🚀 开始严格单因素消融训练... (w/o Focal Loss, seed=42)")
best_val_f1 = 0
patience_counter = 0
history = []
best_model_path = os.path.join(MODEL_SAVE_DIR, "best_model_ablation_no_focal_loss_seed42.pth")
...
for epoch in range(1, EPOCHS + 1):
    ...
    for data in tqdm(train_loader, desc=f"Epoch {epoch:03d}/{EPOCHS}"):
        ...
        # 单因素消融：保留正式模型中的动态负节点采样
        # 所有正节点参与损失；负节点随机保留至多为正节点数量的 UNDERSAMPLE_RATIO 倍。
        pos_mask = (data.y == 1)
        neg_mask = (data.y == 0)
        neg_indices = neg_mask.nonzero(as_tuple=True)[0]

        num_pos = int(pos_mask.sum().item())
        num_neg = min(len(neg_indices), num_pos * UNDERSAMPLE_RATIO)
...
plt.tight_layout()
save_path = os.path.join(MODEL_SAVE_DIR, 'academic_training_curves_ablation_no_focal_loss_seed42.png')
